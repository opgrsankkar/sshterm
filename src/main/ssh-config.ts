...
function insertHost(globalRoot: GroupNode, host: HostEntry, groupPath: string): void {
  ...
  for (let index = 1; index < segments.length; index++) {
    ...
    if (!child) {
      child = { name, path, children: [], hosts: [] }
...
function buildManagedDirBlock(directories: string[]): string[] {
  if (directories.length === 0) return []
  return [
    MANAGED_DIRS_START,
    ...directories.sort((left, right) => left.localeCompare(right)).map((dir) => `# x-sshterm-dir: ${dir}`),
    MANAGED_DIRS_END
  ]
}
...
function rewriteManagedDirBlock(lines: string[], directories: string[]): string[] {
  ...
  const block = buildManagedDirBlock(directories)
...
export async function parseSshConfig(configPath: string): Promise<SshConfigModel> {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    if (dirMatch) {
      const normalized = normalizeGroupPath(dirMatch[1])
      if (normalized && normalized !== 'Global') {
        directoryGroups.add(normalized)
      }
      continue
    }
    ...
    const entry: HostEntry = {
      ...
      assignmentReason: sourceGroupPath
        ? 'valid-comment'
        : activeCommentGroup
          ? 'invalid-comment'
          : 'no-comment'
  ...
  for (const groupPath of directoryGroups) {
...
export async function createGroupInConfig(
  ...
  for (const line of lines) {
    const match = line.trim().match(GROUP_DIR_REGEX)
    if (!match) continue
    const normalized = normalizeGroupPath(match[1])
    if (normalized && normalized !== 'Global') existingDirs.add(normalized)
  }
  existingDirs.add(nextPath)

  const nextLines = rewriteManagedDirBlock(lines, Array.from(existingDirs))
...
export async function deleteGroupInConfig(configPath: string, groupPath: string): Promise<void> {
  ...
  for (const line of lines) {
    ...
    if (!match) continue
    const normalized = normalizeGroupPath(match[1])
    if (!normalized || normalized === 'Global') continue
    if (normalized === normalizedGroup || normalized.startsWith(`${normalizedGroup}/`)) {
      continue
    }
    keptDirs.push(normalized)
  }

  const nextLines = rewriteManagedDirBlock(lines, keptDirs)
  await writeConfigLines(configPath, nextLines)
}
...
export async function moveGroupInConfig(
  ...
  const rewrittenLines = lines.map((line) => {
    ...
    if (dirMatch) {
      ...
      return `# x-sshterm-dir: ${rewritten}`
    }
